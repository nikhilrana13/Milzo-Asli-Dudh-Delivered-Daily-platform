...
import SubscriptionsTable from '@/components/dashboardcomponents/SubscriptionsTable';
import SubsFilterbar from '@/components/dashboardcomponents/SubsFilterbar';
import { useGetVendorSubscriptionsQuery } from '@/redux/api/GetVendorSubscriptionsApi';
import { useGetSubscriptionStatsQuery } from '@/redux/api/SubscriptionStatsApi';
import React, { useState } from 'react';
import { BiCross, BiPauseCircle } from 'react-icons/bi';
...
const Subscriptions = () => {
  ...
  const statsQuery = useGetSubscriptionStatsQuery()
  const subsQuery = useGetVendorSubscriptionsQuery({ page, limit: 6, paymentStatus: selectedPaymentStatus, bookingStatus: selectedBookingStatus })
  const stats = statsQuery?.data?.data?.stats || {}
  const subscriptions = subsQuery?.data?.data?.subscriptions || []
  ...
  const statsData = [
    {
      ...
      value: statsQuery.isError ? "--" : stats?.totalSubs || 0,
      color: "text-blue-600",
      bg: "bg-blue-50",
    },
    {
      ...
      value: statsQuery.isError ? "--" : stats?.activeSubs || 0,
    ...
    },
    {
      ...
      value: statsQuery.isError ? "--" : stats?.cancelledSubs || 0,
      color: "text-red-600",
      bg: "bg-red-50",
    },
    {
      ...
      value: statsQuery.isError ? "--" : stats?.completedSubs || 0,
      color: "text-green-600",
      bg: "bg-green-50",
    },
    {
      ...
      value: statsQuery.isError ? "--" : stats?.pausedSubs ?? 0,
  ...
  const end = Math.min(pagination?.currentPage * pagination?.limit, pagination?.totalSubs)
  return (
    <div className='w-full p-5 flex flex-col gap-8 ' >
      <div className="max-w-2xl">
        ...
        <p className="text-[#3d4a3d] text-sm sm:text-base leading-relaxed">
          Stay on top of your recurring orders. Manage delivery timings, track payment activity,
          and handle pauses efficiently to maintain a smooth{" "}
          <span className="font-medium">subscription lifecycle</span>.
        </p>
      </div>
      {/* subs stats card */}
      ...
      <div className="w-full bg-[#f3f4f6] rounded-xl overflow-hidden">
        {/* filters  */}
        <SubsFilterbar setSelectedPaymentStatus={setSelectedPaymentStatus} setSelectedBookingStatus={setSelectedBookingStatus} />
        {/* table */}
        <SubscriptionsTable subscriptions={subscriptions} isLoading={subsQuery?.isLoading} isError={subsQuery?.isError} />
        {/* pagination */}
        {
          !subsQuery?.isLoading && (
              ...
              <div className="w-full border-t border-[#bccbb9]/30 py-4 px-4 sm:px-6 flex flex-col sm:flex-row gap-3 sm:gap-0 sm:justify-between sm:items-center">
                ...
                <div className="text-center sm:text-left">
                  <span className="text-[#5c5f60] text-xs sm:text-sm font-medium">
                    Showing {start || "0"} – {end || "0"} of {pagination?.totalSubs || 0} Subscriptions
                  </span>
                </div>
                {/* Controls */}
