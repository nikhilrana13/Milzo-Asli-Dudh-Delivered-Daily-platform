                    ...
                    products?.map((product) => (
                        <div key={product?._id} className='bg-white border border-gray-200 rounded-lg p-4 space-y-3'>
                            ...
                            <div className='flex items-center gap-3 pb-3 border-b'>
                                <img src={product?.images?.[0]?.url || noimg} onError={(e) => { e.target.src = noimg; }} className="w-12 h-12 rounded-lg object-cover" />
                            ...
                            <div className='space-y-2'>
                                ...
                                <div>
                                    <p className='text-xs text-gray-600 font-medium'>Pricing</p>
                                    <div className='flex flex-col gap-1'>
                                        {product?.priceOptions?.map((option, idx) => (
                                            <div key={idx} className="flex items-center gap-2 text-xs">
                                                <span className="text-[#006e2f] font-semibold">
                            ...
                            <div className='flex items-center justify-between py-2 border-t border-b'>
                                <div className='flex-1'>
                                    <p className='text-xs text-gray-600 font-medium mb-1'>Stock</p>
                                    <span className={`px-2 py-1 text-xs rounded-full inline-block ${product?.isAvailable ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                                        {product?.isAvailable ? "In Stock" : "Out of stock"}
                                    </span>
                                </div>
                                <div className='flex items-center gap-2'>
                            ...
                            <div className='flex justify-end gap-2 pt-2'>
                                <button onClick={() => onEdit(product)} className="p-2 hover:bg-gray-100 rounded-lg transition">
                                    <MdEdit size={18} />
                                </button>
                                <button disabled={deleteprojectId === product?._id || isDeleting} onClick={() => handleDelete(product?._id)} className="p-2 hover:bg-red-100 text-red-500 rounded-lg transition flex items-center justify-center">
                                    {deleteprojectId === product?._id ? (
                                        <div className="w-4 h-4 border-2 border-red-400 border-t-red-600 rounded-full animate-spin"></div>
                                    ) : (
                                        <MdDelete size={18} />
