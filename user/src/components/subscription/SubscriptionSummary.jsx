...
const SubscriptionSummary = ({ selectProductData, selectedPriceOption }) => {
    ...
    const selectedAddress = userSavedaddresses.find((address) => address?._id === selectedAddressId)
    const { setActiveDialog } = useDialog()
    const [loading, setLoading] = useState(false)
    const navigate = useNavigate()
    // time validation
    const minTime = selectedDeliveryTimings.slot === "morning" ? "06:00" : "16:00"
    const maxTime = selectedDeliveryTimings.slot === "morning" ? "10:00" : "20:00"
    const isValidTime = selectedDeliveryTimings.time >= minTime && selectedDeliveryTimings.time <= maxTime
    // find total price
    ...
    const handleCreateSubscriptionBooking = async () => {
        ...
        if (!isValidTime) {
            return toast.error(selectedDeliveryTimings.slot === "morning" ? "Morning delivery allowed only between 6 AM - 10 AM"
                ...
                <div className="bg-white w-full md:w-[400px] p-6 sm:p-8 rounded-3xl border border-[#eef0f2] shadow-[0_12px_32px_rgba(25,28,30,0.04)] max-h-[calc(100vh-120px)] overflow-y-auto  custom-scrollbar">
                    ...
                    <div className="mb-7">
                        <div className="flex items-center justify-between mb-3">
                            ...
                            <button onClick={() => setActiveDialog("location")} className="text-[#16a34a] text-xs font-semibold hover:underline">
                                Change
                            </button>
                        </div>
                    ...
                    <div className="mb-7">
                        ...
                        <div className="bg-[#f3f4f6] p-4 rounded-2xl space-y-4">
                            ...
                            <div>
                                ...
                                <input type="time" min={minTime} max={maxTime} value={selectedDeliveryTimings.time}
                                    onChange={(e) =>
                                        setSelectedDeliveryTimings((prev) => ({
                                            ...prev,
                                            time: e.target.value
                    ...
                    <div className="grid grid-cols-2 gap-7 mb-8">
                        ...
                        <div>
                            ...
                            <input type="date" value={startDate} min={todayDate} onChange={(e) => setStartDate(e.target.value)} className="rounded-2xl border bg-[#f7f7f7] p-3 text-sm text-gray-500" />
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" min={startDate || todayDate} value={endDate} onChange={(e) => setEndDate(e.target.value)} className="rounded-2xl border bg-[#f7f7f7] p-3 text-sm text-gray-500" />
                        </div>
                    </div>
                    {/* price breakdown */}
                    ...
                    {/* button */}
                    <button onClick={handleCreateSubscriptionBooking} className="w-full py-4 rounded-2xl bg-gradient-to-r from-[#16a34a] to-[#22c55e] text-white
                ...
                showMobileSummary && (
                    <div className="fixed inset-0 z-[70] bg-black/40 pb-28 lg:hidden">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 max-h-[92vh] overflow-y-auto rounded-t-[32px] bg-white p-6 animate-slide-up custom-scrollbar pb-28">
                            ...
                            {/* totals */}
                            <div className="mb-8 rounded-2xl bg-[#fafafa] p-5 space-y-4">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="text-3xl font-black text-[#191c1e]">
                                        ₹{totalAmount}
                                    </span>
                            ...
                            <button
                                ...
                                className="w-full rounded-2xl bg-gradient-to-r from-[#16a34a] to-[#22c55e] py-4 text-lg font-bold text-white"
                            >
                                Proceed to Subscribe
                            </button>
