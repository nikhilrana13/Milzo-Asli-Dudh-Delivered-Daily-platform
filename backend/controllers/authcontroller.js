...
const LoginWithGoogle = async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      user.username = name;
      user.email = email;
      // update google image only if user has not uploaded custom image
...
const adminLogin = async (req, res) => {
  try {
    ...
    return Response(res, 200, "Admin login successful", {
      admin: AdminMapper(admin),
      token,
    });
  } catch (error) {
    return Response(res, 500, "Internal server error");
